...
**Dialogue box**: A framed text box at the bottom of the screen showing one speaker's line, with their Portrait. A presentation layer only — it never implies an interactive NPC; the speaker is whoever the script names. **Always pauses the world and locks the Player** while shown — used by Cutscenes and conversations — and is advanced one line at a time by a dedicated key. The Player's brief, non-pausing self-talk is a _separate_ concept (a **Monologue**), not a mode of this. _Avoid_: Textbox, Speech bubble, Message (a speech bubble floats over a speaker in-world; the Dialogue box is anchored to the screen), and "ambient dialogue" (the non-pausing case is a Monologue, not a Dialogue box).
...
**Monologue**: The Player thinking aloud — a brief, transient line that floats in-world above the Player, drifts up, and fades on a timer, the way the "locked" cue does on a barred Door. It **never pauses** the world, carries no Portrait or box, takes no input (it is not advanced — it simply expires), and only ever speaks in the Player's own voice. Fired fire-and-forget by game events, it is the casual, in-the-moment counterpart to the formal, pausing Dialogue box. _Avoid_: Dialogue box (screen-anchored, pausing, multi-speaker, advance-driven), "ambient dialogue" (the retired name for this), Bark/Aside (industry/theatrical synonyms — the term here is Monologue), Speech bubble.
