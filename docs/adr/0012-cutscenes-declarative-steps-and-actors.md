...
- **Dialogue is a separate system the director merely calls.** Per ADR 0006, the Dialogue box renders in the `UI` scene over the event bus. A `say` step opens a Dialogue line (which pauses `Game`, ADR 0014) and `await`s `dialogue-end` before advancing; a cutscene's conversation is just a run of `say` steps, interleaved with the choreography steps that animate while `Game` is resumed. The text box is built once and shared — the director is one caller, conversation Tripwire handlers are others (the Player's non-pausing self-talk is the separate **Monologue** channel, ADR 0014, not a Dialogue caller). The director owns no text rendering.
