...
- **Dialogue boxes** (always-pausing) plus the Player's transient **Monologue**, and scripted **Cutscenes** — at least an intro and a win cutscene, fired once and tracked in `GameState.progress`
...
- [ ] **Dialogue boxes** — a framed text box at the bottom of the screen with a **Portrait** of the speaker. Purely a **presentation layer** — no interactive NPC entity enters scope; the "speaker" is whatever the script names. Renders in the parallel `UI` scene (extends ADR 0003's UI-scene role to narrative overlays), driven over the event bus (`dialogue-start`/`-advance`/`-end`), never reaching into `Game`. **Always pauses** `Game` and locks the Player while shown (ADR 0014 — the old modal/ambient mode axis is gone; the non-pausing case is now the **Monologue** below); the pause is owned by `Game`, the **dedicated advance key** (distinct from `Space`) by `UIScene` (a paused `Game` can't poll keys). A script is an ordered list of `{ speaker, text }` lines; a **speaker registry** maps `speaker → { name, portrait? }` (Portrait optional — narrator has none; box tolerates a missing one until the Art pass). Text reveals as a **two-stage typewriter** (advance completes the line, then advances).
- [ ] **Monologue** — the Player thinking aloud (CONTEXT.md; ADR 0014): transient world-space text that floats above the Player and fades on a timer, modelled on the existing `RoomManager.denied()` "locked" cue. **Never pauses**, no box/Portrait/advance-key, Player-only, fired fire-and-forget. A separate channel from the Dialogue box, sharing no code.
