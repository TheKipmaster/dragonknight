...
A 2D top-down action dungeon crawler RPG, built with **Phaser 3 + TypeScript + Vite**.
Keyboard movement, mouse-aimed melee. Arcade (AABB) physics.
...
**Controls:** WASD / arrows to move · mouse to aim · Space or left-click to attack
(hold to chain the 3-beat combo).
...
`npm run build` only proves the code *type-checks and bundles*. It can't catch a
runtime crash (a bad Phaser call in a scene's `create()`, a null body, …) or tell
you whether anything actually *works*.
...
5. Runs **behavioural assertions** against the live game — e.g. spawn a `Walker`
   and confirm a hit drops its HP.
...
Bundled modules aren't on `window`, so the test harness can't see `GameState` or
the Phaser instance directly. The game therefore exposes two **dev-only handles**,
gated on a build flag so a real production build never leaks internals:
...
From the test, `page.evaluate(...)` reads/drives the running game through these.
To add an assertion, reach in via `__GAME`/`__STATE`, optionally drive input with
`page.keyboard` / `page.mouse`, then assert on state. See the backlog in
[`docs/ROADMAP.md`](./docs/ROADMAP.md) for assertions worth adding next.
